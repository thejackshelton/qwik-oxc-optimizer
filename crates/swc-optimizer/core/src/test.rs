...
fn example_1() {
	test_input!(TestInput {
		code: r#"import { $, component, onRender } from "@qwik.dev/core";
...
fn example_props_wrapping() {
	test_input!(TestInput {
		code: r#"import { $, component$, useSignal } from "@qwik.dev/core";
...
fn example_reg_ctx_name_segments() {
	test_input!(TestInput {
		code: r#"import { $, component$, server$ } from "@qwik.dev/core";
import { foo } from "./foo";
export const Works = component$((props) => {
  const text = "hola";
  return (
...
fn example_reg_ctx_name_segments_hoisted() {
	test_input!(TestInput {
		code: r#"import { $, component$, server$, useStyle$ } from "@qwik.dev/core";
...
fn example_renamed_exports() {
	test_input!(TestInput {
		code: r#"import { component$ as Component, $ as onRender, useStore } from "@qwik.dev/core";

export const App = Component((props) => {
  const state = useStore({ thing: 0 });

  return onRender(() => <div>{state.thing}</div>);
});
"#
		.to_string(),
		transpile_ts: true,
...
fn issue_117() {
	test_input!(TestInput {
		filename: "project/test.tsx".to_string(),
		code: r#"export const cache = patternCache[cacheKey] || (patternCache[cacheKey] = {});
"#
		.to_string(),
		entry_strategy: EntryStrategy::Single,
		..TestInput::default()
...
fn example_jsx() {
	test_input!(TestInput {
		code: r#"import { $, component$, h, Fragment } from "@qwik.dev/core";
...
export const Foo = component$(
  () => {
    ...
    console.log(qwik);
    const qrl = 23;
    return <div onClick$={() => console.log(qrl)} />;
  },
  {
    tagName: "my-foo",
  },
...
fn example_custom_inlined_functions() {
	test_input!(TestInput {
		code: r#"import { component$, $, useStore, wrap, useEffect } from "@qwik.dev/core";
...
fn example_missing_custom_inlined_functions() {
	test_input!(TestInput {
		code: r#"import { component$ as Component, $ as onRender, useStore, wrap, useEffect } from "@qwik.dev/core";
...
fn example_skip_transform() {
	test_input!(TestInput {
		code: r#"import { component$ as Component, $ as onRender } from "@qwik.dev/core";
...
fn example_use_client_effect() {
	test_input!(TestInput {
		code: r#"import { component$, useBrowserVisibleTask$, useStore, useStyles$ } from "@qwik.dev/core";
...
fn example_parsed_inlined_qrls() {
	test_input!(TestInput {
		code: r#"import { componentQrl, inlinedQrl, useStore, jsxs, jsx, useLexicalScope } from "@qwik.dev/core";
...
fn example_use_server_mount() {
	test_input!(TestInput {
		code: r#"import { component$, useTask$, useStore, useStyles$ } from "@qwik.dev/core";
...
fn example_strip_exports_used() {
	test_input!(TestInput {
		code: r#"import { component$, useResource$ } from "@qwik.dev/core";
import mongodb from "mongodb";
...
fn example_server_auth() {
	test_input!(TestInput {
		code: r#"import GitHub from "@auth/core/providers/github";
...
fn example_strip_client_code() {
	test_input!(TestInput {
		code: r#"import { component$, useClientMount$, useStore, useTask$ } from "@qwik.dev/core";
import mongo from "mongodb";
import redis from "redis";
...
fn issue_476() {
	test_input!(TestInput {
		code: r#"import { Counter } from "./counter.tsx";
...
fn special_jsx() {
	test_input!(TestInput {
		code: r#"// don't transpile jsx with non-plain-object props
...
fn example_mutable_children() {
	test_input!(TestInput {
		code: r#"import { component$, useStore, Slot, Fragment } from "@qwik.dev/core";
...
fn example_immutable_function_components() {
	test_input!(TestInput {
		code: r#"import { component$, useStore, Slot } from "@qwik.dev/core";
...
fn example_build_server() {
	test_input!(TestInput {
		code: r#"import { component$, useStore, isDev, isServer as isServer2 } from "@qwik.dev/core";
...
fn example_derived_signals_div() {
	test_input!(TestInput {
		code: r#"import { component$, useStore, mutable } from "@qwik.dev/core";
...
fn destructure_args_inline_cmp_block_stmt() {
	test_input!(TestInput {
		code: r#"export default ({ data }: { data: any }) => {
  return (
    <div
...
fn destructure_args_inline_cmp_block_stmt2() {
	test_input!(TestInput {
		code: r#"export default (props: { data: any }) => {
  const { data } = props;
  return (
    <div
      data-is-active={data.selectedOutputDetail === "options"}
  ...
  return (
    <div>
      ...
      <div>
        ...
        <button
          onClick$={() => (htmlSignal.value = "<h2><span>I'm a updated signal value!</span></h2>")}
        ></button>
      </div>
    </div>
  );
...
fn example_noop_dev_mode() {
	test_input!(TestInput {
		code: r#"import { component$, useStore, serverStuff$, $ } from "@qwik.dev/core";
...
export const App = component$(() => {
  ...
  return (
    <Cmp>
      <p class="stuff" shouldRemove$={() => stuff.count} onClick$={() => console.log("warn")}>
        Hello Qwik
      </p>
    </Cmp>
  );
...
fn rename_builder_io() {
	test_input!(TestInput {
		code: r#"import { $, component$ } from "@builder.io/qwik";
...
fn example_component_with_event_listeners_inside_loop() {
	test_input!(TestInput {
		code: r#"import { $, component$, useStore, useSignal } from "@qwik.dev/core";
export const App = component$(() => {
...
fn should_wrap_prop_from_destructured_array() {
	test_input!(TestInput {
		code: r#"import { component$, useStore, useTask$ } from "@qwik.dev/core";
...
fn should_mark_props_as_var_props_for_inner_cmp() {
	test_input!(TestInput {
		code: r#"import { component$, useResource$, Resource } from "@qwik.dev/core";
...
fn should_not_wrap_var_template_string() {
	test_input!(TestInput {
		code: r#"import { component$, useComputed$ } from "@qwik.dev/core";
...
export default component$((props) => {
  return <div test="test" {...props}></div>;
});
"#
		.to_string(),
		transpile_ts: true,
...
fn should_not_generate_conflicting_props_identifiers() {
	test_input!(TestInput {
		code: r#"import { component$, useComputed$, useTask$ } from "@qwik.dev/core";
...
fn should_not_move_over_side_effects() {
	test_input!(TestInput {
		code: r#"export const $promoteToRoot$ = (ref: SeenRef) => {
...
fn should_ignore_null_inlined_qrl() {
	test_input!(TestInput {
		code: r#"import { inlinedQrl } from "@qwik.dev/core";
...
fn should_extract_single_qrl() {
	test_input!(TestInput {
		code: r#"import { $, component$, useSignal, Signal } from "@qwik.dev/core";
export const App = component$(() => {
...
const Foo = component$(() => {
  ...
  const Inner = component$((props) => {
    ...
    return (
      <div>
        {data.value.map((item) => (
          <p onClick$={() => console.log(item.value.id)}>{item.value.id}</p>
        ))}
      </div>
    );
  });
...
const Foo = component$(function () {
  ...
  return (
    <div>
      {data.value.map((row, idx) => (
        <div
          onClick$={() => console.log(row.value.id, idx)}
          onMouseOver$={() => console.log("over" + row.value.id)}
        >
          <p onClick$={() => console.log("inner" + row.value.id)}>{item.value.id}</p>
        </div>
